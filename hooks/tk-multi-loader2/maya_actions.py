...
class MayaActions(HookBaseClass):
    ...
    def _create_reference(self, path, sg_publish_data):
        ...
        :param sg_publish_data: Shotgun data dictionary with all the standard publish fields.
        """
        # Sicherstellen, dass Module da sind (zur Sicherheit)
        import maya.cmds as cmds
        import os
        ...
        # --- SEACARUS PIPELINE: USD MODIFICATION START ---
        # Hier prüfen wir, ob es eine USD Datei ist
        if path.lower().endswith((".usd", ".usdc", ".usda")):
            
            # 1. Plugin laden falls nötig
            if not cmds.pluginInfo("mayaUsdPlugin", query=True, loaded=True):
                try:
                    cmds.loadPlugin("mayaUsdPlugin")
                except Exception as e:
                    self.parent.log_error("Could not load mayaUsdPlugin: {}".format(e))
                    return

            # 2. USD Referenz erstellen
            # WICHTIG: type="USD Import" sorgt dafür, dass Maya das USD-Plugin nutzt
            self.parent.log_info("Referencing USD File: {}".format(path))
            cmds.file(
                path, 
                reference=True, 
                type="USD Import", 
            ...
            # WICHTIG: return beendet die Funktion hier, 
            # damit der Code unten NICHT mehr ausgeführt wird.
            return 
        # --- SEACARUS PIPELINE: USD MODIFICATION END ---

        # Now create the reference object in Maya (Standard Code für Maya/Alembic)
        cmds.file(
            path,
            reference=True,
